...
/**
 * Class CTC_PostDataConverter Converts $_POST and $_FILE into the standard format for
 * this plugin to save it
 */
class CTC_PostDataConverter {
     ...
     * @return object that can be passed to ContacticPlugin::saveData()
     */
        ...
        if (is_array($_POST) && !empty($_POST)) {
            ...
            foreach ($_POST as $key => $val) {
                if (!in_array($key, $this->excludeFields)) {
                    $posted_data[$key] = sanitize_text_field($val);
                }
            }
            ...
            if (is_array($_FILES) && !empty($_FILES)) {
                foreach ($_FILES as $key => $file) {
                    if (is_uploaded_file($file['tmp_name'])) {
                        $posted_data[$key] = sanitize_file_name($file['name']);
