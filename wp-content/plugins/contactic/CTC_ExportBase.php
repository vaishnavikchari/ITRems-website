...
require_once('CTC_QueryResultIteratorFactory.php');
require_once('CTC_QueryResultIterator.php');
require_once('CTC_DereferenceShortcodeVars.php');

class CTC_ExportBase {
    ...
    /**
     * @var CTC_Evaluator|CTC_FilterParser|CTC_SearchEvaluator
     */
    var $rowFilter;
    ...
    /**
     * @var CTC_DataIterator|CTC_AbstractQueryResultsIterator
     */
    var $dataIterator;

    function __construct() {
        $this->plugin = new ContacticPlugin();
    }
    ...
    public function dereferenceOption($optionName) {
        if (isset($this->options[$optionName])) {
            $dereferenceVars = new CTC_DereferenceShortcodeVars;
    ...
    public function setCommonOptions($htmlOptions = false) {
        ...
        if ($this->options && is_array($this->options)) {
            ...
            if (isset($this->options['filter']) || isset($this->options['trans'])) {
                ...
                $permittedFunctions = CTC_PermittedFunctions::getInstance();
            ...
            if (isset($this->options['filter'])) {
                require_once('CTC_FilterParser.php');
                $aFilter = new CTC_FilterParser;
                $aFilter->setComparisonValuePreprocessor(new CTC_DereferenceShortcodeVars);
            ...
            if (isset($this->options['search'])) {
                require_once('CTC_SearchEvaluator.php');
                $aFilter = new CTC_SearchEvaluator;
            ...
            if ($numFilters == 1) {
                ...
                require_once('CTC_CompositeEvaluator.php');
                $this->rowFilter = new CTC_CompositeEvaluator;
            ...
            if ($numTransformFilters == 1) {
                ...
                require_once('CTC_CompositeEvaluator.php');
                $this->rowTransformFilter = new CTC_CompositeEvaluator;
                $this->rowTransformFilter->setEvaluators($transformFilters);
            }

            if (isset($this->options['trans'])) {
                require_once('CTC_TransformParser.php');
                $this->transform = new CTC_TransformParser();
                $this->transform->setComparisonValuePreprocessor(new CTC_DereferenceShortcodeVars);
                ...
                if (isset($this->options['orderby'])) {
                    ...
                    foreach ($orderByStrings as $anOrderBy) {
                        ...
                        $transformOption .= '&&CTC_NaturalSortByField(' . $anOrderBy . ',' . $ascOrDesc . ')';
                    }
                }
    ...
    protected function assertSecurityErrorMessage() {
        ...
        $errMsg = $showMessage ? __('You do not have sufficient permissions to access this data.', 'contactic') : '';
        if ($this->isFromShortCode) {
            echo $errMsg;
        }
        else {
            include_once('CTC_Die.php');
            CTC_Die::wp_die($errMsg);
        }
    }
     ...
     * @return void
     */
    protected function setDataIterator($formName, $submitTimeKeyName = null, $merge_col = false, $fields = null) {

        $submitTimes = $this->queryRandomSubmitTimes($formName);

        $sql = $this->getPivotQuery($formName, false, $submitTimes, $merge_col, $fields);
        ...
        $this->dataIterator = CTC_QueryResultIteratorFactory::getInstance()->newQueryIterator($unbuffered);
        ...
        if ($this->transform && !empty($this->transform->transformIterators)) {
            ...
            // in the actual query (CTC_QueryResultIterator) whereas the normally named
...
//    }
     ...
     * @param $merge_col bool
     * @return string
     */
    public function &getPivotQuery($formName, $count = false, $submitTimes = null, $merge_col = false) {
        global $wpdb;
        $tableName = $this->plugin->getSubmitsTableName();
        ...
        // START CONTACTIC Field merger
        $fields_concat = array();

        if ($merge_col) {
            $option_input_merge = $this->plugin->getOption('InputMerge', '%email%,%e-mail%,%e_mail%,%mail%', true);

            // Process and extract merge field inputs from options
            $tmp_merge = explode(',', $option_input_merge);
            $merge_fields = array();

            foreach($tmp_merge as $key => $value) {
                $data = array();
            ...
            // if we have some merge fields, try to remove those from fields
            $field_keys = array();
            foreach($merge_fields as $key => $pattern) {

                if($pattern['match'] == 'contains') {
                    $field_keys = array_merge($field_keys, $this->searchArray($pattern['value'], $fields));
                } else {
                    $field_keys[] = array_search($pattern['value'], $fields);
                }

            }

            $field_keys = array_values(array_filter(array_map('trim', $field_keys), 'strlen'));
        ...
        if ($merge_col && !empty($fields_concat)) {
            $sql .= ", CONCAT_WS(' ' ";
            foreach ($fields_concat as $key => $value) {
                $sql .= ", max(if(`field_name`='".$value."', `field_value`, null )) ";
            }
            $sql .= ") AS 'merge'";
        }
    ...
    public function searchArray($keyword, $arrayToSearch){
    ...
    protected function queryRandomSubmitTimes($formName) {
        ...
        if (isset($this->options['random'])) {
            ...
            if ($numRandom > 0) {
                ...
                $justSubmitTimes = new CTC_ExportBase();
